...
def init_db():
    ...
    if not User.query.filter_by(username='admin@gmail.com').first():
        admin = User(username='admin@gmail.com', display_name='System Admin', password_hash=generate_password_hash('adminpass'), role='admin')
...
def forgot_password():
    ...
    user = User.query.filter_by(username=username).first()
    if not user:
        # Prevent username enumeration by returning generic error or success 
        return jsonify({'success': False, 'error': 'Account not found.'}), 404
        
    user.password_hash = generate_password_hash(new_pwd)
    db.session.commit()
    log_event('password_reset', f"Password reset externally for {username}")
    return jsonify({'success': True})
...
def admin_users():
    if session.get('role') != 'admin':
        log_event('role_violation', f"{session.get('username')} tried to access user management")
        return render_template('error.html', message='Access Denied')
    ...
    user_list = []
    for u in users:
        mem_count = Memory.query.filter_by(owner_username=u.username).count()
        user_list.append({
            'id': u.id,
            'username': u.username,
...
def api_user_delete():
    ...
    if user.username == 'admin' or user.username == session.get('username'):
        return jsonify({'success': False, 'error': 'Cannot delete system admin or self'}), 400
        
    # Delete their memory records too? Or reassign? 
    # For simplicity, let's keep it simple.
    username = user.username
    db.session.delete(user)
    db.session.commit()
    
    log_event('user_deleted', f"User {username} was removed by {session.get('username')}")
    return jsonify({'success': True})
